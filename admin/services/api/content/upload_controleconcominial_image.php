...
if ($file['size'] > $maxBytes) {
    json_response(['ok' => false, 'message' => 'Arquivo muito grande. Maximo 3 MB.'], 400);
}
...
if (!in_array($mime, $allowed_mime, true)) {
    json_response(['ok' => false, 'message' => 'Tipo de arquivo nao permitido. Use JPG, PNG ou WebP.'], 400);
}
...
$filename = 'cd_' . substr($baseName, 0, 40) . '_' . time() . '.' . $ext;

$targetDir = dirname(__FILE__, 5) . '/images/controleacessocondominial/';
$targetPath = $targetDir . $filename;

if (!is_dir($targetDir)) {
    json_response(['ok' => false, 'message' => 'Diretorio de imagens nao encontrado.'], 500);
}
